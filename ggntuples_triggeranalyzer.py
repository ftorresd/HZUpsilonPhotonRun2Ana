...
import config
# config.datasets_data = config.get_datasets_data_trigger()
# config.datasets_mc = config.get_datasets_mc_trigger()
...
#       # os.system("rm -f "+f)
#   # with open("outputs/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + ".coffea", 'wb') as fout:
#     # import cloudpickle
#     # cloudpickle.dump(merged_output, fout)  
#   save(merged_output, "outputs/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + ".coffea")


def merger(ds):
  from tqdm import tqdm
  # print("\n\n--> Merging files for: " + ds["dataset_name"] + " - "+ ds["year"])
  result = subprocess.run("find outputs/buffer/ -type f -name 'trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + "_*.coffea'", shell=True, stdout=subprocess.PIPE)
  # result = subprocess.run("ls outputs/buffer/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + "_*.coffea", shell=True, stdout=subprocess.PIPE)
  # files_list = [load(f) for f in result.stdout.decode('utf-8').splitlines()]
  files_list = []
  ...
  # files_list = [load(f) for f in tqdm(result.stdout.decode('utf-8').splitlines(), desc=ds["dataset_name"] + "_"+ ds["year"], unit=" files")]
  save(utils.accumulator_merger(files_list), "outputs/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + ".coffea")
...
if __name__ == '__main__':
  ...
  if args.test:
    ...
    if config.datasets_data != []:
      if all(e == [] for e in data_errors):
        print("--> No loading errors found for Data.")
      else:
        print("--> Loading errors for Data:")
  ...
  if args.merge:
    ...
    if len(datasets_to_merge_mc) > 1:
      ...
      with ProcessPoolExecutor(max_workers=int(config_doc["n_processors"])) as executor:
    ...
    if len(datasets_to_merge_data) > 1:
      # with ThreadPoolExecutor(max_workers=config_doc["n_processors"]) as executor:
      with ProcessPoolExecutor(max_workers=int(config_doc["n_processors"]/3)) as executor:
  ...
  ############################ 
    ...
    # MC - multiprocess
    for ds in config.datasets_mc:
      print("\n\n--> H/Z --> Upsilon + Photon trigger analyzer for: " + ds["dataset_name"] + " - "+ ds["year"] + " - Chunk: "+ ds["chunk"])
      # clear buffer
      os.system("rm -f outputs/buffer/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + "_"+ ds["chunk"] + "_*.coffea")
      # run the analyzer processor
      output = processor.run_uproot_job(config.get_fileset(ds),
                                    treename='ggNtuplizer/EventTree',
                                    processor_instance=ggntuples_handler.TriggerAnalyzerProcessor(ds["dataset_name"]+"_"+ds["year"]+"_"+ds["chunk"]),
                                    ...
                                    # maxchunks=config_doc["n_processors"],
                                    # chunksize=1000, maxchunks=2,
                                   )
      print("\n\n--> Done with: "+ ds["dataset_name"] + " - "+ ds["year"] + " - Chunk: "+ ds["chunk"])
      # save(output, "outputs/trigger_ana_" + ds["dataset_name"] + "_"+ ds["year"] + ".coffea")
